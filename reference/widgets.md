...
- **柱子收进一个只包住它们的小布局（背景留空），每拍只改序号、最后重绘小布局一次**，
  不要逐根 `ui_pic_show_image_by_id()` —— 每次重绘都遍历整个图层，逐根刷会拖出
  `timer_no_response`。做法和实测数据见 `platform.md` §7「每次 `ui_core_redraw()` 都遍历整个图层」。
